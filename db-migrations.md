...
예를 들어, 개발 중에 새 테이블이 추가 될 수도있을 것이고, 응용 프로그램이 프로덕션으로 마이그레이션 한 후에야 추가 인덱스가 필요하다는 것을 발견 될 수도있을 것입니다.
이러한 데이터베이스의 구조적인 변경 (** 마이그레이션 **라고합니다)를 추적하는 것이 중요하다는 소스 코드에 대한 변경이 버전 관리를 사용하여 추적되는 것과 동일한 것입니다.
소스 코드 및 데이터베이스 동기화가 손실되면 버그가 발생하거나 응용 프로그램 전체가 움직이지 못하게 될까합니다.
이러한 이유로 인해 Yii는 데이터베이스 마이그레이션 도구를 제공하여 데이터베이스 마이그레이션 내역 추적 관리, 새로운 마이그레이션의 적용 또한 기존의 이주를 취소 할 수 있도록하고 있습니다.

다음 단계는 개발 중에 팀에 의해 데이터베이스 마이그레이션이 사용되는 예를 보여줍니다.

1. Tim가 새로운 마이그레이션 (예를 들어, 새 테이블을 만들거나 열 정의를 변경하는 등)을 만든다.
2. Tim이 새로운 마이그레이션을 소스 컨트롤 시스템 (예를 들어 Git이나 Mercurial)에 위탁한다.
3. Doug가 소스 컨트롤 시스템에서 자신의 저장소를 업데이트하여 새 마이그레이션을 받는다.
4. Doug 이주를 자신의 로컬 개발 데이터베이스에 적용하고 Tim이 변경 사항을 반영하여 자신의 데이터베이스를 동기화한다.

Yii는 데이터베이스 마이그레이션을`yii migrate` 명령 줄 도구에서 지원합니다.
...
예를 들어 이주가 * news *라는 테이블을 만들 것 인 경우는 명령을 다음과 같이 사용합니다.

```
yii migrate / create create_news_table
```

즉시 나중에 설명하도록 마이그레이션에서는이`name` 매개 변수는 PHP 클래스 이름의 일부로 사용됩니다.
따라서 알파벳, 숫자 및 / 또는 밑줄 만 포함해야합니다.

위의 명령은`m101129_185401_create_news_table.php`라는 새 파일을 만듭니다.
이 파일은`@ app / migrations` 디렉토리에 생성됩니다.
초기 상태에서는이 마이그레이션 파일은 다음의 코드를 포함하고 있습니다.
...
class m101129_185401_create_news_table extends \ yii \ db \ Migration
{
    ...
    {
        echo "m101129_185401_create_news_table can not be reverted. \ n";
        return false;
    }
}
```

클래스 이름은 파일 이름과 동일하며`m <timestamp> _ <name>`패턴을 따릅니다. 여기에서

*`<timestamp>`이주가 만든 (`yymmdd_hhmmss`라는 서식) UTC 타임 스탬프이며,
*`<name>`명령의`name` 매개 변수에서 가져온 문자열입니다.

클래스 중에서는`up ()`메소드가 실제 데이터베이스 마이그레이션을 구현하는 코드를 포함 할 메소드입니다.
즉,`up ()`메소드가 실제로 데이터베이스를 변경하는 코드를 실행합니다.
`down ()`메쏘드는`up ()`에 의해 변경된 내용을 취소 코드를 포함 할 수 있습니다.

경우에 따라서는`down ()`가 데이터베이스 마이그레이션을 취소 할 수없는 것이 있습니다.
예를 들어 이주가 테이블의 행 또는 테이블 전체를 삭제 한 경우 해당 데이터를`down ()`메소드로 복구 할 수 없습니다.
그런 경우 이주는 돌이킬 수 없다라고 데이터베이스를 이전 상태로 롤백 할 수 없습니다.
마이그레이션이 불가역 인 경우는 생성 된 위의 코드처럼`down ()`메쏘드는`false`을 돌려 이주가 취소 않을 것임을 나타냅니다.

예를 들어, 새 테이블을 만들 마이그레이션을 보여합시다.
